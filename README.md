...
**One thing is still not fixed, and is worth stating plainly:** any sentence that
...
abandons the utterance at that point. It is not the folding in this project; the
same input in plain ASCII is silent too, and `5.19 PM` (digits either side) is
fine, so it is the stop-then-digit shape specifically.
