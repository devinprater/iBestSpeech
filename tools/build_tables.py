...
def sanitise(text):
    ...
    """
    # CLDR's data is XML, so some descriptions carry entities. Unescaping has to
    # happen before anything else, or "&quot;" is stripped to the literal word
    # "andquot" and gets spoken -- which is exactly what the first generation of
    # this table did.
    ...
    # A description that is only punctuation has nothing to say. CLDR has many
    # of these -- the Russian table alone has hundreds of emoji whose annotation
    # is just a dash -- and emitting them would substitute a misreading for
    # silence.
...
def encode_locale(entries):
    """One locale's data as lines of "hex scalars<tab>description".

    Code points are written as hex scalars joined by "-" rather than as literal
    characters: the source stays pure ASCII, the file stays diffable, and a
    multi-scalar key like a family emoji round-trips without depending on how
    Swift chooses to break grapheme clusters.
    """
    lines = []
    for cp in sorted(entries):
        scalars = "-".join("%X" % ord(ch) for ch in cp)
        lines.append("%s\t%s" % (scalars, entries[cp]))
    return "\n".join(lines)
...
def emit_cldr(tables, path):
    ...
    lines.append("// it. XML entities are unescaped, and a description left with nothing")
    lines.append("// but punctuation is dropped rather than spoken as a stray dash.")
    lines.append("")
    ...
    lines.append("        let key = String(character)")
    lines.append("        if base != \"en\", let localized = tables[base]?[key] {")
    lines.append("            return localized")
    lines.append("        }")
    lines.append("        return tables[\"en\"]?[key]")
    lines.append("    }")
    lines.append("")
    lines.append("    /// Every language's table, decoded on first use.")
    lines.append("    ///")
    lines.append("    /// The data ships as one string per language rather than as a dictionary")
    lines.append("    /// literal. %d entries in a single nested literal is one expression" % total)
    lines.append("    /// for Swift to type-check, and it took this project's CI job past its")
    lines.append("    /// timeout -- the build was cancelled in the Swift-tests step. A string")
    lines.append("    /// literal costs nothing to check, and the decode below happens once,")
    lines.append("    /// lazily, on the first character that needs a description.")
    lines.append("    private static let tables: [String: [String: String]] =")
    lines.append("        rawTables.mapValues(decode)")
    lines.append("")
    lines.append("    /// One line per entry: the code points as hex scalars joined by \"-\",")
    lines.append("    /// a tab, then the description. A tab cannot appear in either, because")
    lines.append("    /// the key is hex and the description is sanitised ASCII.")
    lines.append("    private static let rawTables: [String: String] = [")
    for locale in sorted(tables):
        lines.append("        %s: %s," % (swift_literal(locale),
                                         swift_literal(encode_locale(tables[locale]))))
    lines.append("    ]")
    lines.append("")
    lines.append("    private static func decode(_ data: String) -> [String: String] {")
    lines.append("        var out: [String: String] = [:]")
    lines.append("        for line in data.split(separator: \"\\n\", omittingEmptySubsequences: true) {")
    lines.append("            guard let tab = line.firstIndex(of: \"\\t\") else { continue }")
    lines.append("            var scalars: [Unicode.Scalar] = []")
    lines.append("            for part in line[line.startIndex..<tab].split(separator: \"-\") {")
    lines.append("                if let value = UInt32(part, radix: 16),")
    lines.append("                   let scalar = Unicode.Scalar(value) {")
    lines.append("                    scalars.append(scalar)")
    lines.append("                }")
    lines.append("            }")
    lines.append("            guard !scalars.isEmpty else { continue }")
    lines.append("            out[String(String.UnicodeScalarView(scalars))]")
    lines.append("                = String(line[line.index(after: tab)...])")
    lines.append("        }")
    ...
    print("  wrote %s (%d lines, %d locales, %d entries)"
          % (path.name, len(lines), len(tables), total))
